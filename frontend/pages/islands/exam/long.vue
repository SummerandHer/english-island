<template>
  <div class="space-y-4">
    ...
    <header class="flex flex-wrap items-end justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold">长篇阅读</h1>
        <p class="mt-1 text-sm text-gray-500">信息匹配 · 完整段落点选 · 考试模式提交前不可点词</p>
      </div>
      <NSelect
    ...
    <div v-else-if="!rows.length" class="island-card p-6 text-sm text-gray-500">暂无长篇，请稍后再来。</div>
...
async function load() {
  loading.value = true
  try {
    const q = new URLSearchParams({ sectionType: 'long_match' })
