<template>
  <div class="space-y-4">
    ...
    <div v-else-if="!rows.length" class="island-card p-6 text-sm text-gray-500">
      {{ emptyText }}
    </div>
...
const props = defineProps<{
  sectionType: 'short_careful' | 'long_match'
  title: string
  subtitle: string
  emptyText: string
  /** Extra text after examLevel in badge, e.g. " · 匹配" */
  badgeSuffix?: string
}>()
...
function badgeText(p: SimPassageCard) {
  const level = p.examLevel?.toUpperCase() ?? ''
  return props.badgeSuffix ? `${level}${props.badgeSuffix}` : level
}

async function load() {
  loading.value = true
  try {
    const q = new URLSearchParams({ sectionType: props.sectionType })
